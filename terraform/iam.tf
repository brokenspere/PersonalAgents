...
# Screener Lambda IAM Role
resource "aws_iam_role" "screener_lambda_role" {
  name = "${var.project_name}-screener-role-${var.environment}"
...
resource "aws_iam_role_policy_attachment" "screener_basic_execution" {
  role       = aws_iam_role.screener_lambda_role.name
  policy_arn = "arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole"
}

resource "aws_iam_policy" "screener_sqs_policy" {
  name        = "${var.project_name}-screener-sqs-policy-${var.environment}"
  description = "Allows screener lambda to publish to notification queue"
...
resource "aws_iam_role_policy_attachment" "screener_sqs_attachment" {
  role       = aws_iam_role.screener_lambda_role.name
  policy_arn = aws_iam_policy.screener_sqs_policy.arn
}

resource "aws_iam_policy" "screener_ssm_policy" {
  name        = "${var.project_name}-screener-ssm-policy-${var.environment}"
  description = "Allows screener lambda to read SSM parameters"
...
resource "aws_iam_role_policy_attachment" "screener_ssm_attachment" {
  role       = aws_iam_role.screener_lambda_role.name
  policy_arn = aws_iam_policy.screener_ssm_policy.arn
}
