...
class ScreenedTickerItem(BaseModel):
    ...
    upcoming_earnings_date: Optional[str] = Field(None, description="Upcoming earnings date if within next 7 days")
    event_risk: bool = Field(False, description="True if there's a macro event risk in the next 7 days")

class ScreenedPayload(BaseModel):
    """
    Data published by ScreenerAgent with LLM swing trade plan.
    """
    source: str = Field(..., description="Source of the screening, e.g., 'swing_trade_screener'")
    market: str = Field(..., description="Market identifier")
    timestamp: datetime = Field(default_factory=utc_now)
    screened_tickers: List[ScreenedTickerItem] = Field(default_factory=list, description="Tickers that passed the technical criteria")
    trading_plan: Optional[str] = Field(None, description="Thai language trading plan generated by LLM")
